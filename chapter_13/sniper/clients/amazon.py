...
class ClientAmazon:
    ...
    async def get_async_cookies(self) -> bool:
        url = "https://www.amazon.com"
    ...
    async def product_details(self, product_url: str):
        cookies = await self.get_async_cookies()
        async with httpx.AsyncClient() as client:
            r = await client.get(product_url, headers=self.headers)
