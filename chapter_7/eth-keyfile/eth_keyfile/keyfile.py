...
def extract_key_from_keyfile(path_or_file_obj: Union[str, IO[str]], password: bytes) -> bytes:
...
def _scrypt_hash(password: bytes, salt: bytes, n: int, r: int, p: int, buflen: int) -> bytes:
    derived_key = scrypt(
...
def _pbkdf2_hash(password: bytes, hash_name: str, salt: bytes, iterations: int, dklen: int) -> bytes:
...
def encrypt_aes_ctr(value: Union[bytes, bytearray, memoryview], key: bytes, iv: int) -> bytes:
    ctr = Counter.new(128, initial_value=iv, allow_wraparound=True)
    encryptor = AES.new(key, AES.MODE_CTR, counter=ctr)
